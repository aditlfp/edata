...
function IndexTempUsers(props) {
    ...
    const submit = (e) => {
        e.preventDefault();
         try {
            router.post(route('accept-employe.update', ids), 
            {
                _method: 'PATCH',
                status: 1,
            },
  ...
  return (
      <AdminLayout>
            ...
            <table className="table table-zebra table-xs w-full mb-5">
                    <thead className="sticky top-0">
                        <tr className="bg-orange-600 text-white capitalize">
                            <th className="border-x-[1px] border-orange-300 sticky top-0 text-center">
                               <input type="checkbox" className='checkbox checkbox-warning checkbox-sm' checked={isChackedAll} onChange={(e) => handleCheckboxChange(e)} />
                            </th>
                            <th className="border-x-[1px] border-orange-300 sticky top-0 text-center">
                               #
                            </th>
                   ...
                   <tbody>
                        <EachUtils colspan={8} of={paginatedData} render={(item, i) => {
                            return (
                            <tr key={i}>
                                <td className="border-x-[1px] border-orange-300 text-center">
                                    <input type="checkbox" className='checkbox checkbox-warning checkbox-sm' checked={checkedItems.includes(item.id)} onChange={() => handleCheck(item.id)}/>
                                </td>
                                <td className="border-x-[1px] border-orange-300 text-center">
                                    {i + 1}
                                </td>
                                <td className="border-x-[1px] border-orange-300 text-center">
                                    <div className='ml-6'>
                                        <img src={`https://absensi-sac.sac-po.com/public/storage/user/`+item.image} alt={item.nama_lengkap} className="w-10 h-10 rounded-full" />
                                    </div>
                                </td>
                                <td className="border-x-[1px] border-orange-300 text-center">
                                    {item.nama_lengkap}
...
                                <td className="border-x-[1px] border-orange-300 text-center">
                                    {item.status == 0 ? 
                                    <span className='badge badge-sm rounded-sm badge-warning text-white p-2'>Pending</span>
                                    : item.status == 1 ? 
                                     <span className='badge badge-sm rounded-sm badge-success text-white p-2'>Approve</span>
                                    :  <span className='badge badge-sm rounded-sm badge-error text-white p-2'>Rejected</span>}
                                </td>
                                <td className='text-center flex justify-center items-center gap-2'>
                                    {/* Add your action buttons here */}
                                    <button disabled={item.status === 1} onClick={() => accept(item.id)} className='btn btn-sm btn-success rounded-sm text-white text-xl'><BiCheckDouble /></button>
                                    <button disabled={item.status === 1 || item.status === 2 } onClick={() => reject(item.id)} className='btn btn-sm btn-warning rounded-sm text-white text-xl'><MdCancel /></button>
                                    <button onClick={() => removeUser(item)} className='btn btn-sm btn-error rounded-sm text-white text-lg'><FaTrashCan /></button>
                                </td>
                            </tr>
                            )
                        }}/>
